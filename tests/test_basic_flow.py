...
BASE = "http://localhost"
TEST_UID = "basic_test_" + str(int(time.time()))[-6:]
CREATED_UIDS = []
...
def api(method, path, data=None):
    ...
    if data is not None:
        ...
        # Flask get_json() 被调用两次，{} 会导致第二次返回 None
...
def test_doc_delete_and_appears_in_trash():
    """创建文档后通过 API 删除，验证出现在删除页"""
    uid = TEST_UID + "_d"
    CREATED_UIDS.append(uid)
    api('POST', '/api/docs', {"uid": uid, "title": "API删除测试文档", "body": "body",
                              "tags": [], "annotations": [], "created": "2026-04-07"})
    del_result = api('DELETE', f'/api/docs/{uid}')

    # 确认删除成功（返回 moved_to_trash）
    assert del_result.get('moved_to_trash') or del_result.get('ok'), \
        f"DELETE失败: {del_result}"
    ...
    with sync_playwright() as p:
        ...
        assert any("API删除测试文档" in t for t in trash_titles), \
            f"删除页找不到: {trash_titles}"
        browser.close()
...
def run_all():
    ...
    tests = [
        ...
        test_trash_page_loads,
        test_doc_create_and_appears_in_list,
        test_doc_delete_and_appears_in_trash,
        test_trash_view_modal,
        test_trash_restore_doc,
    ]
    failed = 0
    for t in tests:
        try:
    ...
    if failed == 0:
        ...
        print(f"通过 {len(tests)-failed}/{len(tests)}，失败 {failed} 个 ❌")
    print("="*50)
    return failed == 0
